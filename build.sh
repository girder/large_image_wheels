...
docker run -v `pwd`/wheels:/opt/mount --rm --entrypoint bash girder/large_image_wheels -c 'cp --preserve=timestamps /io/wheelhouse/{libtiff,Glymur,psutil,GDAL,mapnik,openslide_python,pyvips,pyproj,pylibmc,python_javabridge}*-cp*many* /opt/mount/. && chown '`id -u`':'`id -g`' /opt/mount/*.whl'
