...
docker run -v `pwd`/wheels:/opt/mount --rm --entrypoint bash girder/large_image_wheels -c 'cp --preserve=timestamps /io/wheelhouse/{psutil,javabridge,libtiff,Glymur,openslide_python,GDAL,mapnik,pyvips,pyproj,pylibmc}*many* /opt/mount/. && chown '`id -u`':'`id -g`' /opt/mount/*.whl'
