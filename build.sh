...
docker run -v `pwd`/docs:/opt/mount --rm --entrypoint bash manthey/large_image_wheels -c 'cp /io/wheelhouse/{GDAL,libtiff,mapnik,openslide_python,pyvips,psutil,ujson}*many* /opt/mount/. && chown '`id -u`':'`id -g`' /opt/mount/*.whl'
